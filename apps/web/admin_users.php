...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($action==='create'){
  ...
  if($name===''||!filter_var($email,FILTER_VALIDATE_EMAIL)||strlen($password)<8)$error='Informe nome, e-mail válido e senha com pelo menos 8 caracteres.';
   ...
   try{$st=$pdo->prepare('INSERT INTO users(name,email,password_hash,role) VALUES(?,?,?,?)');$st->execute([$name,$email,password_hash($password,PASSWORD_DEFAULT),$role]);$msg='Usuário criado.';}
   catch(PDOException $e){$error='Não foi possível criar o usuário. Verifique se o e-mail já existe.';}
  }
 }elseif($action==='toggle'){
  $id=(int)($_POST['user_id']??0);
  if($id!==$me['id']){$pdo->prepare('UPDATE users SET active=IF(active=1,0,1) WHERE id=?')->execute([$id]);$msg='Situação alterada.';}
 }elseif($action==='role'){
  $id=(int)($_POST['user_id']??0);$role=($_POST['role']??'user')==='admin'?'admin':'user';
  if($id!==$me['id']){$pdo->prepare('UPDATE users SET role=? WHERE id=?')->execute([$role,$id]);$msg='Perfil alterado.';}
 }
}
